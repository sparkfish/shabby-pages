...
\item The DDI-100 corpus \cite{ddi-100-2019} is larger in scale than \emph{NoisyOffice} and the DIBCO datasets, but is again limited in the degree of feature diversity \cite{detection-masking-2022}.
\end{itemize}
...
Another dataset, DDI-100 \cite{ddi-100-2019}, was constructed in a manner similar to \emph{NoisyOffice}.
DDI-100 is far larger than \emph{NoisyOffice}, but has been criticized for not containing enough diversity or noise to be an effective benchmark \cite{detection-masking-2022}.
Additionally, the pipeline code for creating the noisy versions of both \emph{NoisyOffice} and DDI-100 is not available, and thus these datasets cannot be reproduced.

\begin{figure}
    \centering\scalebox{0.4}{
    \includegraphics{figures/dataset_examples.pdf}}
    \caption{Example noisy-clean pairs from PHIBD (top row), DIBCO-13 (middle row), and \emph{NoisyOffice} (bottom row).}
...
Similar to \emph{NoisyOffice}, the DIBCO datasets do not contain graphical elements like images, tables, charts, etc.
...
\begin{table}[]
    ...
    \begin{tabular}{lcllllllll}
        ...
        \rot{80}{\textbf{Dataset Size}}
            ...
            \midrule
         \emph{ShabbyPages} (ours)~ & 6,202~ & \cmark & \cmark & 0.488~ & multi~ & multi~ & \cmark & \cmark & \cmark \\
         \emph{NoisyOffice} \cite{ref_NoisyOffice} & 72  & \cmark & \cmark & 0.317 & 16pt & 1 & \xmark & \xmark & \xmark \\
...
\begin{table}[]
    ...
    \begin{tabular}{lllll}
    \toprule
    \textbf{Training Set~~} & \textbf{Test Set} & \textbf{SSIM$\uparrow$~} & \textbf{PSNR$\downarrow$~} \textbf{RMSE$\downarrow$~}\\
        \midrule
